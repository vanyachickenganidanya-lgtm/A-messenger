...
def convert_8k_to_48k(data_8bit):
    ...
    for b in data_8bit:
        ...
        for _ in range(6):
            out.extend(sample_bytes * 2)
    return bytes(out)

def convert_48k_to_8k(data_48k):
    out = bytearray()
    for i in range(0, len(data_48k), 24):
        if i + 1 < len(data_48k):
...
async def on_ready():
    ...
    if voice_channel:
        ...
        print(f"Успешно подключились к голосовому каналу: {VOICE_CHANNEL_NAME}")
